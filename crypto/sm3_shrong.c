...
int crypto_sm3_update(struct shash_desc *desc, const u8 *data,
			  unsigned int len)
{
	return 0;	//sm3_base_do_update(desc, data, len, sm3_generic_block_fn);
}
...
int crypto_sm3_final(struct shash_desc *desc, u8 *out)
{
	...
	return 0;	//sm3_base_finish(desc, out);
}
...
{
	crypto_sm3_update(desc, data, len);
...
MODULE_DESCRIPTION("SM3 Secure Hash Algorithm Using Shrong Card");

MODULE_ALIAS_CRYPTO("sm3");
MODULE_ALIAS_CRYPTO("sm3-shrong");
